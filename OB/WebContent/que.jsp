						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach var="vo" items="${list}">
									<tr>
										<td>${vo.id}</td>
										<td><a href="controller?type=queView&board_type=1&id=${vo.id}&quePage=${quePvo.nowPage}">${vo.title}"></a></td>
										<td>관리자</td>
							...
							<td colspan="4">
								...
								<!-- [이전으로]에 대한 사용 여부를 처리 -->
								 <c:choose>
									...
									<!-- 사용가능(enable): 두번째 블록 이상(첫번째만 아닌경우)  -->
									<c:otherwise>
									<li>
										<a href= "controller?type=que&board_type=1&quePage=${quePvo.beginPage - 1 }">이전으로</a>
									</li>
									</c:otherwise> 
								</c:choose> 
								...
								<c:forEach var="k" begin="${quePvo.beginPage }"
									...
									<c:choose>
										<c:when test="${k == quwPvo.nowPage}">
											<li class="now">${k}
											</li>
										</c:when>
										<c:otherwise>
											<li>
											<a href="controller?type=que&board_type=1&quePage=${k}">${k}></a>
											</li>
										</c:otherwise>
									</c:choose>
									</c:forEach>
									...
									<c:choose>
										...
										<c:otherwise>
											<li><a
												href="controller?type=que&board_type=1&quePage=${que.endPage + 1 }">다음으로</a>
											</li>
										</c:otherwise>
									</c:choose>
								</ol>
