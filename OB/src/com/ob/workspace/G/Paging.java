...
public class Paging {
	...
	private int numPerPage = 3;	//한 페이지당 게시글 수
	private int pagePerBlock = 3;	//한 블록당 표시하는 페이지 수
	
	private int totalRecord = 0;	//총 게시물 갯수(원본 게시글 수)
	private int totalPage = 0;	//전체 페이지 갯수
	private int totalBlock = 0;	//전체 블록 갯수
	
	private int begin = 0;	//현재 페이지상의 시작 글번호(동적생성 row번호)
	private int end = 0;	//현재 페이지상의 끝 글번호(동적생성 row번호)
	
	private int beginPage = 0;	//현재 블록의 시작 페이지 번호
	private int endPage = 0;	//현재 블록의 끝 페이지 번호
	
	//전체 페이지 구하기
	//totalRecord 값을 페이지당 표시할 글의 갯수 값으로 나누고,
